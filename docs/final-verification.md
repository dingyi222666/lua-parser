...
| --- | --- | --- |
| At least 500 new tests exist (source accounting) | **Met (source count)** | `docs/test-strategy.md` + `NewTestInventoryTddTest` bars: campaign **307** `*TddTest.kt` files / **4857** `@Test` methods (excl. `testinventory`); baseline **366** / **5378**. `remaining_to_500 = 0`. Source count is necessary; suite pass is separate. |
