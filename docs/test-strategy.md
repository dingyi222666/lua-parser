...
`src/jvmTest/kotlin` on 2026-07-13 (TASK-677 WINSLICE-29220594557 s018 live
recount; prior TASK-125 GOAL-PATH snapshot 331/4640 and 272/4125 raised) using
the same counting rules as `testinventory.NewTestInventoryTddTest`. It excludes
...
| --- | --- | ---: | ---: | --- |
| Parser/source | JVM | 87 | 793 | Lua 5.3/5.4/AndroLua syntax, parser recovery, comments, regression fixtures, literal comments, Android-Lua grammar, AST visitor/modifier behavior, AST to Lua round trips, lexer corpora. |
...
| Semantic core | JVM | 141 | 2357 | Binder, declarations, position queries, comment interop, doc comment parsing, type model/syntax/resolution, call/member/return checking, public API compatibility, Android-Lua interop semantics, LuaJava helper typing. |
...
| Workspace | JVM | 37 | 494 | JVM workspace updates, query facade behavior, Android-Lua import workspace behavior, builtin overlay loading, dirty-set planning, module graph cycles, path styles, fingerprint stability, semantic workspace campaign coverage. |
| Interop | JVM | 17 | 375 | JVM class module provider, reflection-backed workspace modeling, Java class metadata, classloader configuration, Android jar reflection, package enumeration, luajava import modeling. |
| LSP | JVM | 64 | 1112 | Language service, lifecycle diagnostics, navigation symbols, text document service, hover/completion/definition/declaration/references/signature help, shutdown/idempotency, watched files/workspace folders, Android-Lua JVM resolution paths. |
| Integration | JVM | 4 | 43 | Android-Lua corpus semantic verification across parser, workspace, and interop fixtures; mixed LuaJava integration coverage. |
| **Total current inventory** | Common + JVM | **366** | **5378** | Current parser, semantic, workspace, interop, LSP, and integration coverage, excluding the inventory fixture. |
...
As of 2026-07-13 (TASK-677 WINSLICE-29220594557 s018 live recount; not a global
success claim), the current source-tree accounting reports:

```text
current_campaign_files = 307
current_campaign_total = 4857
remaining_to_500 = 0
```
...
TASK-677 raised both this document and the inventory fixture constants to the
live source-tree counts (baseline 366/5378, campaign 307/4857) for Windows s018
file drift (+35 baseline/campaign files vs prior 331/272). Bars were only
raised; `testinventory` remains excluded from campaign totals. Area
classification maps `/parser/`, `/source/`, and `/lexer/` into parser so no
counted suite lands in `other`. Verification remains deferred to TASK-043.
