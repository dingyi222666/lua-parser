...
| `textDocument/prepareCallHierarchy` / `callHierarchy/incomingCalls` / `outgoingCalls` | Yes — boolean `true` | Yes | No | Advertised + wired | Same-file local-function call graph subset; non-function positions → empty. |
