export const meta = {
  ...
  description: 'Watch slice → merge bar → ≤30 workers → re-run SAME until green else next',
  phases: [
    ...
    { title: 'Push', detail: 'Commit; re-run same if red, else next' },
  ],
}
...
const push = await agent(
  [
    'ROLE: Commit slice progress + product fixes. GATE must-green-to-advance.',
    'REPO: ' + REPO,
    'BRANCH MUST BE windows-verify. NEVER main.',
    'RUN_ID: ' + RUN_ID,
    'WAVE: ' + WAVE,
    'Merge status context: ' + JSON.stringify(merge || {}),
    '',
    'GATE RULES (strict):',
    '- Read tasks/agent-runs/win-slices/progress.json after merge.',
    '- Head = first slice in slices.json order whose status != success.',
    '- If head is failure/pending/running: after workers re-trigger SAME head only.',
    '- Only if head is success (or all success): push may advance to next pending.',
    '- NEVER mark a red slice success. NEVER skip a red slice.',
    '',
    'Steps:',
    '1. git checkout windows-verify if needed; git status -sb',
    '2. Stage progress bar + results + product fixes + task files:',
    '   tasks/agent-runs/win-slices/progress.json, PROGRESS.md, results/**',
    '   tasks/agent-runs/REVIEW-SLICE-* / WORKER-IDS',
    '   product src/ + tasks/TASK-*.md from workers',
    '3. Do NOT stage .claude/, org/, handoff noise, sh.exe.stackdump, gradlew.lf/unix',
    '4. Commit message:',
    '   - if slice RED: fix(windows): slice <id> reds run ' + RUN_ID + ' (gate hold)',
    '   - if slice GREEN: ci(windows): slice <id> green run ' + RUN_ID + ' (advance)',
    '5. git push origin windows-verify (workflow auto-picks head again)',
    '6. If no code changes but need re-run same red: gh workflow run windows-jvmtest.yml --ref windows-verify',
    '7. gh run list --branch windows-verify --workflow=windows-jvmtest.yml --limit 2',
    '',
    'FORBIDDEN: push main; force push; advancing by editing red to success; skipping head.',
    'Return {pushed, commit, newRunId, gate, headSlice, headStatus, notes}',
  ].join('\n'),
  {
    label: 'push-gate-slice',
    phase: 'Push',
    model: 'sonnet',
    effort: 'medium',
    schema: {
      ...
      properties: {
        ...
        gate: { type: 'string' },
        headSlice: { type: 'string' },
        headStatus: { type: 'string' },
        notes: { type: 'string' },
      },
      required: ['pushed', 'notes', 'gate', 'headSlice', 'headStatus'],
      additionalProperties: true,
    },
  }
)
