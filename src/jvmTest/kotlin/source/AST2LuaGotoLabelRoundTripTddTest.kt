...
class AST2LuaGotoLabelRoundTripTddTest {
    ...
    @Test
    fun roundTripsForwardAndBackwardAcrossNestedVisibility() {
        assertRoundTrips(
            listOf(
                Sample(
                    source = """
                ...
                Sample(
                    ...
                    printedFragments = listOf("goto inner", "::inner::", "local x = 1")
                ),
                Sample(
                    source = """
                        ::L::
    ...
    @Test
    fun ifBodiesWithLabelsAssertFragmentsWithoutTrailingEndRequirement() {
        // Pure-if printer omits trailing end; assert shape + fragments only.
        val samples = listOf(
            Sample(
        ...
        samples.forEach { sample ->
            ...
            assertTrue(
                ...
                "IfStatement print must not emit trailing end:\n$printed"
            )
        }
    }
    ...
    @Test
    fun collectsForwardAndBackwardLabelNamesAfterRoundTrip() {
        val source = """
            ::root::
            while keep do
              ::loop::
