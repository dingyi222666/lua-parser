...
class LuaLexerLongStringEdgeTddTest {
    ...
    @Test
    fun partialCloseNoiseAndExactLevelClosesDoNotTruncateEarly() {
        assertTokenCases(
            LexerCase(
            ...
            LexerCase(
                "level-1 ignores ]= without final ] then closes",
                "[=[keep ]= still]=]",
                token(LuaTokenTypes.LONG_STRING, "[=[keep ]= still]=]")
            ),
            LexerCase(
            ...
            LexerCase(
                "level-3 ignores lower and partial equals closes",
                "[===[keep ]=== ]==] ]=] ]] end]===]",
                token(LuaTokenTypes.LONG_STRING, "[===[keep ]=== ]==] ]=] ]] end]===]")
            ),
            LexerCase(
    ...
    @Test
    fun operatorCallAndTableAdjacencyKeepStableBoundaries() {
        assertTokenCases(
            LexerCase(
                "string-call style adjacency",
                "print[[hello]]",
                token(LuaTokenTypes.NAME, "print"),
                token(LuaTokenTypes.LONG_STRING, "[[hello]]")
            ),
            LexerCase(
                "method-call style colon name then long string",
                "obj:msg[=[body]=]",
                token(LuaTokenTypes.NAME, "obj"),
                token(LuaTokenTypes.COLON, ":"),
                token(LuaTokenTypes.NAME, "msg"),
                token(LuaTokenTypes.LONG_STRING, "[=[body]=]")
            ),
            LexerCase(
                "table field long string value",
                "{k=[[v]],}",
            ...
            LexerCase(
                "parens around long string keep parentheses",
                "([[x]])",
            ...
            LexerCase(
                "concat chain of two long strings",
                "[[a]]..[=[b]=]",
                token(LuaTokenTypes.LONG_STRING, "[[a]]"),
                token(LuaTokenTypes.CONCAT, ".."),
            ...
            LexerCase(
                "assignment RHS long string then number",
                "s=[====[payload]====]1",
            ...
            LexerCase(
                "return long string keeps keyword boundary",
                "return[==[ok]==]",
    ...
    @Test
    fun incompleteOpenersAndCloseFragmentsDoNotBecomeLongStrings() {
        assertTokenCases(
            LexerCase(
            ...
            LexerCase(
                "bracket triple-equals without second bracket",
                "[===",
                token(LuaTokenTypes.LBRACK, "["),
                token(LuaTokenTypes.EQ, "=="),
                token(LuaTokenTypes.ASSIGN, "=")
            ...
            LexerCase(
                "higher close fragment outside long brackets stays operators",
                "]===]",
                token(LuaTokenTypes.RBRACK, "]"),
                token(LuaTokenTypes.EQ, "=="),
                token(LuaTokenTypes.ASSIGN, "="),
            ...
            LexerCase(
                "well-formed then incomplete open stays split",
                "[[]][=",
                token(LuaTokenTypes.LONG_STRING, "[[]]"),
                token(LuaTokenTypes.LBRACK, "["),
                token(LuaTokenTypes.ASSIGN, "=")
    ...
    fun unclosedLongStringsRecoverWithoutHangAtLevels0Through4() {
        ...
        assertEofMalformedConsumesAll(
            "unclosed level-3 with CR/LF and lower closes",
            "[===[x\r\n]=] ]==] still open"
        )
        assertEofMalformedConsumesAll(
    ...
    fun unclosedLongStringDoesNotHangOnLargeBody() {
        ...
        assertTrue(
            elapsedMs < 5_000L,
            "Unclosed level-4 large edge body must finish without hang; elapsedMs=$elapsedMs"
        )
        result.onSuccess { tokens ->
            assertTrue(
                tokens.any { it.type == LuaTokenTypes.BAD_CHARACTER },
                "Expected BAD_CHARACTER for unclosed large edge body, got $tokens"
            )
            val firstBad = tokens.indexOfFirst { it.type == LuaTokenTypes.BAD_CHARACTER }
            assertTrue(
                tokens.drop(firstBad + 1).isEmpty(),
                "Unclosed large edge body must not invent tokens after BAD_CHARACTER: $tokens"
            )
        }.onFailure { failure ->
            assertTrue(
                failure is IllegalStateException || failure is IllegalArgumentException,
                "Expected lexer exception for unclosed large edge body, got ${failure::class.simpleName}: ${failure.message}"
            )
        }
    }
    ...
    fun levelMismatchAndPartialCloseRecoverWithoutHang() {
        assertMalformedCases(
            ...
            MalformedCase(
                "level mismatch after leading newline body",
                "[=[\nmismatch]==]"
            )
        )
        assertEofMalformedConsumesAll(
