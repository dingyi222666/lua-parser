...
class Lua53BitwiseOpsCorpusTddTest {
    ...
    @Test
    fun exposesLiteralOperandsOnBitwiseExpressions() {
        val expression = assertIs<BinaryExpression>(parseExpression("0x0F << 2 | 1"))
        assertEquals(ExpressionOperator.BIT_OR, expression.operator)

        val shift = assertIs<BinaryExpression>(expression.left)
        assertEquals(ExpressionOperator.BIT_LT, shift.operator)
        val leftLiteral = assertIs<ConstantNode>(shift.left)
        assertEquals("0x0F", leftLiteral.rawValue.toString())
        val shiftCount = assertIs<ConstantNode>(shift.right)
        assertEquals("2", shiftCount.rawValue.toString())

        val rightLiteral = assertIs<ConstantNode>(expression.right)
        assertEquals("1", rightLiteral.rawValue.toString())

        val hexLadder = assertIs<BinaryExpression>(parseExpression("0xFF & 0xF0 | 0x0A"))
        assertEquals(ExpressionOperator.BIT_OR, hexLadder.operator)
        val hexAnd = assertIs<BinaryExpression>(hexLadder.left)
        assertEquals(ExpressionOperator.BIT_AND, hexAnd.operator)
        assertEquals("0xFF", assertIs<ConstantNode>(hexAnd.left).rawValue.toString())
        assertEquals("0xF0", assertIs<ConstantNode>(hexAnd.right).rawValue.toString())
        assertEquals("0x0A", assertIs<ConstantNode>(hexLadder.right).rawValue.toString())
    }

    @Test
    fun roundTripsNestedBitwiseMixesWithStableShapes() {
        val samples = listOf(
            "return a << b << c",
            "return a >> b >> c",
            "return a & b & c",
            ...
            "return flags & mask << offset == expected",
            "return ready and flags | mask or fallback",
            "return ((a | b) & c) << (d + e)",
            "return ~base ^ exp & mask",
        ...
        val failures = samples.mapNotNull { source ->
            runCatching {
                ...
                // Printed source should still contain the core bitwise operators from the input.
    ...
    @Test
    fun parsesBitwiseCorpusAsSuccessfulChunkBodies() {
        val sources = listOf(
            "local mask = 0xFF << 4",
            "local flags = ~ready & mask | toggle",
            "return a | b ~ c & d << e, a << b >> c",
            "if flags & MASK ~= 0 then return true end",
            "while ready and bits << 1 | 1 do break end",
            "for i = 1, limit do total = total | (1 << i) end",
            "function pack(a, b) return (a & 0xFF) | (b << 8) end",
            "local t = { a & b, c | d, ~e, f << g }",
            "repeat x = x >> 1 until x & 1 == 0",
            "do local n = ~~seed | 1; return n & mask end",
            "local function unpack(v) return v & 0xFF, v >> 8 & 0xFF end",
        ...
        sources.forEach { source ->
            ...
            // renderShape must not throw for any successfully parsed bitwise corpus case
            assertTrue(renderShape(chunk).isNotBlank())
        }
    }

    @Test
    fun documentsBitwiseInStatementContextsWithShapes() {
        val localMask = parse(LuaVersion.LUA_5_3, "local mask = 0xFF << 4")
        assertEquals(
            "Chunk(Block[Local(Id(mask)=Binary(<<,Const(0xFF),Const(4)))])",
            renderShape(localMask)
        )

        val ifBitwise = parse(LuaVersion.LUA_5_3, "if flags & MASK ~= 0 then return true end")
        assertEquals(
            "Chunk(Block[If(Clause(Binary(~=,Binary(&,Id(flags),Id(MASK)),Const(0)):Block[Return(Const(true))]))])",
            renderShape(ifBitwise)
        )

        val assignOr = parse(LuaVersion.LUA_5_3, "total = total | (1 << i)")
        assertEquals(
            "Chunk(Block[Assign(Id(total)=Binary(|,Id(total),Binary(<<,Const(1),Id(i))))])",
            renderShape(assignOr)
        )
