...
data class WorkspacePublicFingerprint(
    ...
    companion object {
        ...
        private fun serializeSurface(surface: ModuleExportSurface): String = buildString {
            ...
            append('#')
            append(serializeModuleType(surface.moduleType))
        }

        private fun serializeModuleType(moduleType: ModuleType): String = buildString {
            append("name=")
            append(moduleType.moduleName)
            append(';')
            append("fields=")
            append(sortedEntries(moduleType.fields).joinToString("|") { (name, type) ->
                "$name:${serializeType(type)}"
            })
            append(";methods=")
            ...
            append(moduleType.indexSignature?.let {
                "${serializeType(it.keyType)}->${serializeType(it.valueType)}"
            }.orEmpty())
        }

        private fun serializeTypeParameters(typeParameters: List<TypeParameterType>): String =
            typeParameters.joinToString("|") { serializeType(it) }

        private fun serializeType(type: Type): String = when (type) {
            is ModuleType -> "module(${serializeModuleType(type)})"
            is TableType -> buildString {
                append("table(fields=")
                append(sortedEntries(type.fields).joinToString("|") { (name, nested) ->
                    "$name:${serializeType(nested)}"
                })
                append(";methods=")
            ...
            is FunctionType -> buildString {
                ...
                append(serializeTypeParameters(type.typeParameters))
                append(";parameters=")
                append(type.parameters.joinToString("|") { parameter ->
                    buildString {
                        append(parameter.name)
                        append(':')
                        append(serializeType(parameter.type))
                        append(':')
                ...
                append(")->")
                append(serializeType(type.returnType))
            }

            is OverloadedFunctionType -> "overload(${type.callSignatures.joinToString("|") { serializeType(it) }})"
            is CallableType -> "callable(${type.callSignatures.joinToString("|") { serializeType(it) }})"
            is UnionType -> "union(${type.types.map(::serializeType).sorted().joinToString("|")})"
            is IntersectionType -> "intersection(${type.types.map(::serializeType).sorted().joinToString("|")})"
            is TupleType -> "tuple(${type.elementTypes.joinToString("|") { serializeType(it) }})"
            is MultiReturnType -> "multi(${type.types.joinToString("|") { serializeType(it) }})"
            is VarargType -> "vararg(${serializeType(type.elementType)})"
            is ArrayType -> "array(${serializeType(type.elementType)})"
            is ClassType -> buildString {
                append("class(")
                append(type.name)
                append(";superClass=")
                append(type.superClass?.let(::serializeType).orEmpty())
                append(";superType=")
                append(type.superType?.let(::serializeType).orEmpty())
                append(";typeParameters=")
                append(serializeTypeParameters(type.typeParameters))
                append(";alias=")
                append(type.alias?.let(::serializeType).orEmpty())
                append(";fields=")
                append(sortedEntries(type.fields).joinToString("|") { (name, nested) ->
                    "$name:${serializeType(nested)}"
                })
            ...
            is AliasType -> "alias(${type.name}:${serializeType(type.target)})"
            is TypeParameterType -> "typeparam(${type.name}:${type.constraint?.let(::serializeType).orEmpty()}:${type.defaultType?.let(::serializeType).orEmpty()})"
            is AppliedType -> "applied(${type.baseName}:${type.typeArguments.joinToString("|") { serializeType(it) }})"
...
private fun sortedEntries(map: Map<String, Type>): List<Map.Entry<String, Type>> =
